...
public class FileCopy extends JFrame implements ActionListener {
	...
	private void fileCopy(String path) throws IOException{
		...
		try {
			...
			byte[] readData1 = new byte[512];
			while((readCnt = fis.read(readData1))!=-1) {
				cnt++;
			}//512byte씩 읽어들였을 때 반복 횟수
			fis.close(); //stream은 한 번 진행하면 뒤로 돌아가지 않는다.
			fis = new FileInputStream(originalFile); //file 포인터를 처음으로 옮기기 위해 재연결
			...
			while((readCnt = fis.read(readData))!=-1) {
				...
				jpb.setValue(temp*100/cnt); //현재 반복 수가 전체수의 몇 %인지 연산하여 설정
